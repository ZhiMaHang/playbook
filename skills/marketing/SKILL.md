---
name: marketing
description: 对营销站按一份 SEO/GEO/SEM 任务清单做全面扫描与自动整改——线上与源码双向核对，能在代码层解决的直接改好，改不了的输出带操作指引的人工待办。用法：/marketing <站点标识|all> [只扫描] [部署]。凡是用户要求给某个营销站做 SEO/GEO/SEM 检查、收录体检、营销扫描、新功能上线后的营销跟进、「看看 xx 站还缺什么营销动作」时，都用本技能，即使用户没有说出 /marketing 这个词。站点信息取自本技能 references/sites.local.md（首次使用需按 sites.md 模板填写）。
---
...
1. **站点档案**：本技能目录 `references/sites.local.md` —— 各站仓库路径、线上 URL、关键文件位置、内容来源、部署方式、收录渠道现状。**不存在时不要猜测站点信息**，让用户先照 `references/sites.md` 模板填一份。发现档案与仓库实际不符时，以仓库为准并顺手修正档案。
